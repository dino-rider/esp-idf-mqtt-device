...
// added config_congiguration
void config_configuration(esp_mqtt_client_config_t &mqtt_client_cfg, Configuration const &config)
{
    mqtt_client_cfg.lwt_topic = config.session.last_will.lwt_topic.has_value() ? config.session.last_will.lwt_topic.value().c_str() : NULL;
    mqtt_client_cfg.lwt_msg = config.session.last_will.lwt_msg.has_value() ? config.session.last_will.lwt_msg.value().c_str() : NULL;
    mqtt_client_cfg.lwt_qos = config.session.last_will.lwt_qos;
    mqtt_client_cfg.lwt_retain = config.session.last_will.lwt_retain;
    mqtt_client_cfg.lwt_msg_len = config.session.last_will.lwt_msg_len;
}

// added display function to debug config
void display_config_lwt(esp_mqtt_client_config_t const &mqtt_client_cfg)
{
    printf("lwt_topic: %s \n",mqtt_client_cfg.lwt_topic);
    printf("lwt_msg: %s \n",mqtt_client_cfg.lwt_msg);
    printf("lwt_qos: %d \n",mqtt_client_cfg.lwt_qos);
    printf("lwt_retain: %d \n",mqtt_client_cfg.lwt_retain);
}
...
esp_mqtt_client_config_t make_config(BrokerConfiguration const &broker, ClientCredentials const  &credentials, Configuration const &config)
{
    ...
    config_configuration(mqtt_client_cfg, config);
